        ...
        <div className="py-6" >
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
                ...
                <ul role="list" className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 mt-5">
                    ...
                    {friendRequests.length ? friendRequests.map((fr, index) => (
                        <li key={index} className="col-span-1 bg-white rounded-lg shadow divide-y divide-gray-200">
                            <div className="w-full flex items-center justify-between p-6 space-x-6">
                                <div className="flex-1 truncate">
                                    <div className="flex items-center space-x-3">
                                        <h3 className="text-gray-900 text-sm font-medium truncate">{fr.name}</h3>
                                        <span className="flex-shrink-0 inline-block px-2 py-0.5 text-green-800 text-xs font-medium bg-green-100 rounded-full">
                                            online
                                        </span>
                                    </div>
                                    <p className="mt-1 text-gray-500 text-sm truncate">{fr.email}</p>
                                </div>
                                <img
                                    className="w-10 h-10 bg-gray-300 rounded-full flex-shrink-0"
                                    src={fr.avatar || 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=facearea&facepad=2&w=256&h=256&q=80'}
                                    alt=""
                                />
                            </div>
                            <div>
                                <div className="-mt-px flex divide-x divide-gray-200">
                                    <div className="w-0 flex-1 flex">
                                        <a
                                            href={`/friend-request/accept/${fr.username}`}
